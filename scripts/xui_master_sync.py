...
stream_settings = {repr(stream_settings)}

conn = sqlite3.connect('{server["db_path"]}')
cursor = conn.cursor()
cursor.execute("UPDATE inbounds SET settings = ?, stream_settings = ? WHERE protocol = 'vless'",
               (json.dumps(settings), json.dumps(stream_settings)))
conn.commit()
conn.close()
print(f"Updated {{len(settings['clients'])}} users")
'''
